    ...
    public HttpResponseMessage Get(int departureCity, int arrivalCity, DateTime departureDate)
    {
      ...
      if (departureDate == DateTime.Today)
      {
          ...
          query = $@"
              ...
              left join registration on route.id = registration.route_id
              where DATEDIFF(hour,route.start_time, '{departureDate.ToString("yyyy-MM-dd" + " " + DateTime.Now.ToString("HH:mm:ss.mmm"))}') <= -1 and ac1.id = {departureCity} and ac2.id = {arrivalCity}
              ...
              HAVING aircraft.business_seats + aircraft.economy_seats - COUNT(registration.seat) > 0";
      }
      else
      {
        query = $@"       
