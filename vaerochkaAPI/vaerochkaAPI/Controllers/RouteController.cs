    ...
    public HttpResponseMessage Get(int departureCity, int arrivalCity, DateTime departureDate)
    {
      ...
      if (departureDate == DateTime.Today)
      {
          ...
          query = $@"
              ...
              where DATEDIFF(hour,route.start_time, '{departureDate.ToString("yyyy-MM-dd" + " " + DateTime.Now.ToString("HH:mm:ss.mmm"))}') <= -2 and ac1.id = {departureCity} and ac2.id = {arrivalCity}
